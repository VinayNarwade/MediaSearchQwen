...
class Qwen3VLReranker():
    ...
    def process(
        ...
        final_scores = []
        p = 1
        # parallel processing can be added here for speedup
        def process_single_pair(pair, pair_idx):
            inputs = self.tokenize([pair])
            inputs = inputs.to(self.model.device)
            scores = self.compute_scores(inputs)
            return pair_idx, scores

        with ThreadPoolExecutor(max_workers=16) as executor:
            futures = {executor.submit(process_single_pair, pair, idx + 1): idx for idx, pair in enumerate(pairs)}
            results = [None] * len(pairs)
            for future in as_completed(futures):
                pair_idx, scores = future.result()
                results[pair_idx - 1] = scores

        for scores in results:
            final_scores.extend(scores)
        # for pair in pairs:
        #     print( "Processing pair:", p )  # Debug print
        #     t1 = time.time()
        #     inputs = self.tokenize([pair])
        #     inputs = inputs.to(self.model.device)
        #     scores = self.compute_scores(inputs)
        #     t2 = time.time()
        #     print("Time taken for computing scores for pair", p, ":", t2 - t1)
        #     final_scores.extend(scores)
        #     p += 1
        return final_scores